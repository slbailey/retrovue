...
Activate the venv (do this first, in every shell where you run Core Python):
  From repo root:    source server/.venv/bin/activate
  From server/:    source .venv/bin/activate

Then, with the venv active:
  Install deps:      pip install -r server/requirements.txt   (from repo root)
  Run all tests:    pytest server/tests/
  Run one file:     pytest server/tests/path/to/test_file.py -vv
  Lint:             ruff check server/src/   ;  mypy server/src/
...
Do NOT:
- Run `python`, `pytest`, or `pip` for Core without activating server/.venv first.
