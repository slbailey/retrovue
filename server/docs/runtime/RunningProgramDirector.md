...
2. Set the binary path explicitly:  
   ...
   then start Program Director. The launcher looks for, in order: `RETROVUE_AIR_EXE`, `runtime/build/retrovue_air`, `runtime/out/build/linux-debug/retrovue_air` (relative to repo root when run from the repo).
...
```bash
# From repo root; vcpkg and protos set up (see runtime README)
...
The binary is `runtime/build/retrovue_air`. Program Director looks for it (or `RETROVUE_AIR_EXE`); it runs the binary, talks gRPC, and consumes the TS stream via the accepted UDS connection.

## How to run Air-only and verify

1. **Build Air** (see above). Ensure `runtime/build/retrovue_air` exists and is executable, or set `RETROVUE_AIR_EXE` to its path.
...
| Build Air               | Build `runtime`; required for GET /channel/{id}.ts.      |
