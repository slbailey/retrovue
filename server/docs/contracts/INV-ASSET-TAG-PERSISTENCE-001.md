...
- `server/src/retrovue/domain/entities.py` — `AssetTag` model, `Asset.tags` relationship
- `server/src/retrovue/domain/tag_normalization.py` — normalization primitives
- `server/src/retrovue/cli/commands/_ops/collection_ingest_service.py` — ingest-time persistence
- `server/src/retrovue/cli/commands/asset.py` — operator-time persistence (`asset update --tags`)
- `server/alembic/versions/*_add_asset_tags_table.py` — schema migration
...
- `server/tests/contracts/runtime/test_inv_asset_tag_persistence.py`
