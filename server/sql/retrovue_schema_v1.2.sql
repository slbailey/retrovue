...
CREATE UNIQUE INDEX uidx_content_episode
  ON content_items(show_id, season_number, episode_number)
  WHERE kind='episode' AND show_id IS NOT NULL;
...
CREATE TABLE schedule_instances (
  ...
  air_date TEXT NOT NULL,   -- 'YYYY-MM-DD'
  start_time TEXT NOT NULL, -- 'HH:MM:SS'
  end_time TEXT NOT NULL,   -- 'HH:MM:SS'
  content_item_id INTEGER REFERENCES content_items(id) ON DELETE SET NULL,
  show_id INTEGER REFERENCES shows(id) ON DELETE SET NULL,
...
CREATE TABLE play_log (
  id INTEGER PRIMARY KEY,
  channel_id INTEGER NOT NULL REFERENCES channels(id) ON DELETE CASCADE,
  ts_start TEXT NOT NULL,  -- ISO8601 TEXT
  ts_end TEXT,             -- ISO8601 TEXT
