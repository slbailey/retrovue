...
- **Air:** `runtime/docs/contracts/**`
- **Core:** `server/docs/contracts/**` + `docs/core/`, `server/docs/runtime/`, `server/docs/scheduling/`
...
| **LAW-002** | PlayoutEngineContract legacy preload RPC; FileProducerContract PROD-010 | No explicit test that asserts "no output after hard_stop_time_ms". | test_file_producer_hard_stop_respected, test_playout_no_output_after_hard_stop | Log when producer clamped at boundary |
...
| **LAW-006** | PlayoutInvariants §5; OutputContinuityContract | PTS monotonicity stated; legacy switch RPCResponse.pts_contiguous optional in Phase 6A. | test_output_pts_monotonic, test_switch_pts_contiguous (when implemented) | (none) |
...
| **AIR-005** | PlayoutEngineContract; ProducerBusContract | shadow_decode_started, pts_contiguous optional in 6A; not asserted. | Assert legacy preload RPCResponse.shadow_decode_started when applicable; legacy switch RPCResponse.pts_contiguous when implemented | legacy preload RPCResponse.shadow_decode_started, legacy switch RPCResponse.pts_contiguous |
...
| **AIR-010** | ScheduleManager INV-PLAYOUT-SWITCH-BEFORE-EXHAUSTION | Core has INV-PLAYOUT-*; no test that asserts legacy preload RPC timestamp < boundary. | test_core_prefeed_before_boundary | Log legacy preload RPC/legacy switch RPC call timestamps |
| **AIR-013** | OutputTimingContract; legacy T-* | OutputTimingContract defines behavior; legacy T-002, T-003, T-005, T-007 tests (test_sink_timing_*) not in runtime. | test_sink_timing_pts_mapping, test_sink_timing_early_frames, test_sink_timing_late_drops, test_sink_timing_stop | late_frame_drops |
...
| **OBS-001** | PlayoutEngineContract §legacy preload RPC, §legacy switch RPC | shadow_decode_started, pts_contiguous are optional; no MUST. | Upgrade to SHOULD with test when implemented | legacy preload RPCResponse.shadow_decode_started, legacy switch RPCResponse.pts_contiguous |
