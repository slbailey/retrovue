...
// Purpose: Fail if runtime code (runtime/src, runtime/include) uses float FPS timebase
...
// Resolve runtime root: prefer RETROVUE_AIR_SOURCE_DIR; else assume cwd is build dir and parent is air root.
fs::path GetAirRoot() {
  ...
  // When run via ctest from runtime/build, cwd is build; parent is runtime.
