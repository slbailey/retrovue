...
Contract tests under `runtime/tests/contracts/` can run in **fast deterministic mode** so CI and local runs finish in minutes instead of 15–30+ minutes.
...
- **contract** — Default for CI. All non-soak tests have this label. Run: `ctest --test-dir runtime/build -L contract`
