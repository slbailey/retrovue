...
- After editing a source file, check that the object file was actually rebuilt:
    ...
    ls -la runtime/build/CMakeFiles/retrovue_air.dir/src/producers/file/FileProducer.cpp.o
    # Build
    cmake --build runtime/build -j$(nproc)
    # After build - timestamp MUST have changed
    ls -la runtime/build/CMakeFiles/retrovue_air.dir/src/producers/file/FileProducer.cpp.o
- If the object file timestamp didn't change, force recompilation:
    touch runtime/src/producers/file/FileProducer.cpp
    cmake --build runtime/build -j$(nproc)
- For a full clean rebuild of a specific target:
    rm runtime/build/retrovue_air
    cmake --build runtime/build -j$(nproc)
- Do NOT assume the build worked without verifying timestamps changed.

Build directory:
- The build directory MUST be runtime/build.
- Do NOT use: build/ at repo root, runtime/out/, runtime/build-asan, or any other path.
- Do NOT run cmake from inside runtime/build; always use -S runtime -B runtime/build from repo root (or -S . -B build from runtime).

Output binary:
- The binary MUST be runtime/build/retrovue_air.
- Core spawns this binary when running `retrovue start`.
- Do NOT build or use binaries from any other location.

Logs:
- AIR writes logs to runtime/logs/<channelname>-air.log (e.g., runtime/logs/cheers-24-7-air.log).
...
- Python stubs go to server; C++ is generated by this project’s CMake.
