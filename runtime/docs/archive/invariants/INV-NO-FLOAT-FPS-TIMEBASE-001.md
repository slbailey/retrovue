...
**Status:** Active  
**Owner:** All runtime code under `runtime/src` and `runtime/include`  
...
3. **Exceptions**
   - Code under `runtime/tests` or explicitly documented as test-only / diagnostic-only may use float FPS timebase math for synthetic data or telemetry display, provided it is not used to drive output scheduling or PTS in production paths.
...
`test_inv_no_float_fps_timebase_001.cpp` scans all source files under `runtime/src` and `runtime/include` and fails if any line (excluding comments) matches the forbidden patterns. A small allowlist can be used for known-safe occurrences; default is none.
