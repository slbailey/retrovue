...
|----------|--------|----------|
| Is **PumpDecoderOnce** present, correct, and used in the right place? | Present and correct; **not used** in production. | `FFmpegDecoder::PumpDecoderOnce` implemented in `runtime/src/decode/FFmpegDecoder.cpp:1121–1354`. Adapter forwards in `FFmpegDecoderAdapter.cpp:43–44`. No callers in `VideoLookaheadBuffer.cpp`, `PipelineManager.cpp`, or `TickProducer.cpp`. Only tests and reference doc reference it. |
...
- `runtime/src/blockplan/FFmpegDecoderAdapter.cpp` — PumpDecoderOnce forward
- `runtime/src/blockplan/VideoLookaheadBuffer.cpp` — FillLoop, space_cv_.wait, burst/bootstrap
- `runtime/src/blockplan/PipelineManager.cpp` — TickLoop cadence repeat/advance, audio pop
- `runtime/src/blockplan/TickProducer.cpp` — TryGetFrame → DecodeFrameToBuffer
- `runtime/src/decode/FFmpegDecoder.cpp` — DecodeFrameToBuffer, ReadAndDecodeFrame, PumpDecoderOnce
- `runtime/include/retrovue/blockplan/ITickProducerDecoder.hpp` — PumpMode, PumpResult, PumpDecoderOnce
- `runtime/docs/contracts/semantics/INV-AUDIO-LIVENESS.md` — Contract definition
- `runtime/docs/PHASE2_DECODER_REFERENCE.cpp` — Intended refactor (ReadAndDecodeFrame using pending queue)
