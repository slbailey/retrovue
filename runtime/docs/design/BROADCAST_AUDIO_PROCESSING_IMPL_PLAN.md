...
**Authority:** `runtime/docs/design/BROADCAST_AUDIO_PROCESSING.md` (revised 2026-03-12)
**Date:** 2026-03-12
...
**New file:** `runtime/include/retrovue/blockplan/BroadcastAudioProcessor.hpp`
...
**Modified file:** `runtime/include/retrovue/blockplan/PipelineManager.hpp`
...
**Modified file:** `runtime/src/blockplan/PipelineManager.cpp`
...
**Modified file:** `runtime/CMakeLists.txt`
...
| File | Type | Purpose |
|------|------|---------|
| `runtime/include/retrovue/blockplan/BroadcastAudioProcessor.hpp` | Header | Processor class definition and implementation |
| `runtime/tests/contracts/BlockPlan/BroadcastAudioProcessorContractTests.cpp` | Test | Contract tests for INV-BROADCAST-DRC-001 through 004 |

### Modified files

| File | Change |
|------|--------|
| `runtime/include/retrovue/blockplan/PipelineManager.hpp` | Add `#include`, `broadcast_audio_processor_` member, diagnostic accumulators |
| `runtime/src/blockplan/PipelineManager.cpp` | Three insertions: tick loop Process call + diagnostics, segment advance log + Reset, block activation Reset |
| `runtime/CMakeLists.txt` | Add test file to `blockplan_contract_tests` sources |
...
| Verification | Method |
|--------------|--------|
| Existing `LoudnessGainContractTests` still pass | `ctest --test-dir runtime/build -R LoudnessGain` |
| Existing `AudioLookaheadBufferTests` still pass | `ctest --test-dir runtime/build -R AudioLookahead` |
| Full `blockplan_contract_tests` suite green | `ctest --test-dir runtime/build -R BlockPlanContracts` |
