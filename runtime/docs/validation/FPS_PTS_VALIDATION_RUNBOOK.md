...
For **each** test title, record the following. Logs are in `runtime/logs/<channel>-air.log` for the run where that title was first content.
...
**Run:** HBO, first content = Close Encounters (23.976→29.97). Log: `runtime/logs/hbo-air.log` (SESSION_BUILD build_ts=2026-03-12T21:01:54). **Note:** This report is from the run *before* the 1-based source_frame_index fix. After restart with the fixed binary, re-run and replace with new evidence.
...
**Bundled mpegts muxer (force PCR):** The repo patches `runtime/third_party/ffmpeg/src/libavformat/mpegtsenc.c` so that in `mpegts_init`, if options did not enable PCR (`pcr_period_ms < 0` or `mux_rate <= 1`), we force `pcr_period_ms = 20` and `mux_rate = 8000000`. That makes the muxer always use the CBR PCR path. **You must rebuild the static FFmpeg** so this patch is in `libavformat.a`, then rebuild AIR. From repo root: `bash runtime/scripts/build_ffmpeg_static.sh` (if you hit a GCC ICE, try `cd runtime/third_party/ffmpeg/src && make -j1` after configure), then `cmake --build runtime/build --target retrovue_air`.
...
When investigating fast playback, capture and inspect these three log sequences (run with Close Encounters as first content, then grep `runtime/logs/<channel>-air.log`).
