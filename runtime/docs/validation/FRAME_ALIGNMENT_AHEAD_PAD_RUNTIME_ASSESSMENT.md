...
**Log source:** `runtime/logs/hbo-air.log` (latest run).
...
1. Start the channel that exhibits intermittent black (e.g. HBO or the channel whose log is `runtime/logs/<channel>-air.log`).
...
4. Stop the run. The AIR log is at `runtime/logs/<channel>-air.log` (e.g. `runtime/logs/hbo-air.log`).
...
```bash
python3 runtime/scripts/collect_pad_evidence.py runtime/logs/<channel>-air.log
```

Example for HBO:

```bash
python3 runtime/scripts/collect_pad_evidence.py runtime/logs/hbo-air.log
```

For CSV (e.g. for spreadsheets or further analysis):

```bash
python3 runtime/scripts/collect_pad_evidence.py runtime/logs/hbo-air.log --csv > pad_evidence.csv
```
