...
**Build invariants:** See [Build invariants](../build.md) for non‑negotiable rules (Air owns codecs, FFmpeg static, no LD_LIBRARY_PATH, `runtime/scripts/build_ffmpeg_static.sh` is authoritative).
...
- Update `protos/playout.proto` (repo root) when the control API evolves.
- Regenerate bindings:

```bash
# Canonical proto: protos/playout.proto (repo root). C++ is generated by runtime CMake.
# Python: sh scripts/air/generate_proto.sh  (output: server/src/retrovue/proto/)
protoc --proto_path=protos --cpp_out=... --grpc_out=... --plugin=protoc-gen-grpc=`which grpc_cpp_plugin` protos/playout.proto
python3 -m grpc_tools.protoc -I protos --python_out=server/src/retrovue/proto --grpc_python_out=server/src/retrovue/proto protos/playout.proto
```
