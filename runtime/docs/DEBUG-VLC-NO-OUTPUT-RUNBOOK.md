...
- `runtime/include/retrovue/output/OutputBus.h` — dbg counters, heartbeat time
- `runtime/src/output/OutputBus.cpp` — RouteVideo/RouteAudio heartbeat, attach/detach logs
- `runtime/src/output/MpegTSOutputSink.cpp` — enqueue heartbeat, drop detection, mux state, write heartbeat, bootstrap write
- `runtime/src/runtime/PlayoutEngine.cpp` — `[DBG-SWITCH]` auto_completed logs
- `runtime/src/renderer/ProgramOutput.cpp` — `[DBG-PO]` SetOutputBus (already present)
