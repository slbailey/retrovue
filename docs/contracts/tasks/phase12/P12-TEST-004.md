...
- Verify no new LoadPreview scheduled
- Verify no new SwitchToLive scheduled
...
With `_teardown_pending` set, Core MUST NOT schedule new boundary work: no new LoadPreview, SwitchToLive, or segment planning.
...
2. Test no new LoadPreview when pending:
   - Initial state: `_teardown_pending = True`
   - Action: Attempt to schedule LoadPreview (via whatever mechanism triggers it)
   - Expected: LoadPreview NOT scheduled; log indicates skip

3. Test no new SwitchToLive when pending:
   - Initial state: `_teardown_pending = True`
   - Action: Attempt to schedule SwitchToLive
   - Expected: SwitchToLive NOT scheduled; log indicates skip
