...
- Add guard in any method that schedules new LoadPreview or SwitchToLive
...
3. In any method that explicitly schedules LoadPreview:
   - Check `_teardown_pending` before scheduling
   - If true: skip scheduling, log debug

4. In any method that explicitly schedules SwitchToLive:
   - Check `_teardown_pending` before scheduling
   - If true: skip scheduling, log debug
...
- LoadPreview callback fires while teardown pending: let it execute
...
- In `tick()`, after grace timeout check (P12-CORE-004): if `_teardown_pending`, log debug "Skipping boundary work (teardown pending)", return early. No new LoadPreview/SwitchToLive scheduling.
