...
2. When a boundary is skipped:
   - No legacy preload RPC is issued for that boundary
   - No legacy switch RPC is scheduled for that boundary
...
- **Implementation:** `channel_manager.py`
  ...
  - Added `_advance_to_next_boundary_after_skip(skipped_boundary_time)`: uses `get_playout_plan_now(channel_id, skipped_boundary_time)` to set `_segment_end_time_utc` and `_plan_boundary_ms` from next segment (or None). No legacy preload RPC/legacy switch RPC for skipped boundary; current segment continues.
