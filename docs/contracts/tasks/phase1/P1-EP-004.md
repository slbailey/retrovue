...
Assertions Required:
1. [x] Verify codec_ctx->max_b_frames == 0 after encoder init (implied by output check; codec_ctx not exposed)
2. [x] Encode 60 video frames
3. [x] Inspect each output packet's pict_type (decode via avcodec to get pict_type)
4. [x] Assert no packet has pict_type == AV_PICTURE_TYPE_B
5. [x] Confirm mix of I and P frames present (not all I-frames)
...
Status: **Complete** — TEST_INV_ENCODER_NO_B_FRAMES_001 in MpegTSPlayoutSinkContractTests.cpp
