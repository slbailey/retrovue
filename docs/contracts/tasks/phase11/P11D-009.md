...
Scheduling feasibility MUST be determined exactly once, during planning, before any execution-time control commands (including legacy preload RPC or legacy switch RPC) are issued.
...
- All boundaries entering execution are feasible by construction (boundary_time ≥ planning_time + MIN_PREFEED_LEAD_TIME)
