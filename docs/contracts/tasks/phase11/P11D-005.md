...
Instructions:
- Remove legacy switch RPC retry loop
...
Core MUST NOT poll/retry legacy switch RPC based on readiness. If AIR returns NOT_READY or PROTOCOL_VIOLATION, it represents a Core timing bug.
...
1. Remove retry loop from switch_to_live:
   ...
   async def switch_to_live(self, channel_id: int, plan_handle: str) -> bool:
       ...
       if response.status == SwitchStatus.OK:
           ...
           # This is a Core timing bug — we issued legacy switch RPC too late
...
2. Add timing assertion before issuing switch:
   ```python
   def _build_switch_request(self, channel_id: int, plan_handle: str) -> legacy switch RPCRequest:
       ...
       return legacy switch RPCRequest(
...
Constraints:
- NO retry loops for legacy switch RPC
...
Done Criteria:
Core never retries legacy switch RPC; unexpected responses raise exceptions with violation logs.

---
**Done:** channel_manager_launch: SwitchTimingError, SwitchProtocolError; air_switch_to_live returns (ok, result_code, violation_reason) and raises on PROTOCOL_VIOLATION/NOT_READY. Launch path: single legacy switch RPC call, no retry. channel_manager.switch_to_live: single call, raises on SwitchTimingError/SwitchProtocolError.
