...
- Track actual lead time for all legacy preload RPC calls
...
1. Define histogram metric:
   ...
   prefeed_lead_time_ms = Histogram(
       ...
       'Lead time in milliseconds between legacy preload RPC and target boundary',
       ['channel_id'],
   ...
   prefeed_lead_time_violations_total = Counter(
       ...
       'Count of legacy preload RPC calls with insufficient lead time',
       ['channel_id']
   )

   switch_lead_time_ms = Histogram(
       'retrovue_switch_lead_time_ms',
       'Lead time in milliseconds between legacy switch RPC and target boundary',
       ['channel_id'],
       buckets=PREFEED_LEAD_TIME_BUCKETS
   )
   ```
...
Constraints:
- Metric MUST be recorded for every legacy preload RPC call
...
Done Criteria:
Histogram metric records prefeed lead time for all legacy preload RPC calls; visible in Prometheus/Grafana.

---
**Done:** `metrics.py` defines `prefeed_lead_time_ms = Histogram("retrovue_prefeed_lead_time_ms", ...)` and `switch_lead_time_ms`. Observed in `switch_to_live()` when metrics available. Test `test_switch_lead_time_observed_when_metrics_available` verifies recording.
