...
Instructions:
- Populate target_boundary_time_ms in legacy switch RPCRequest
...
1. In ChannelManager.switch_to_live() or equivalent:
   ...
   request = legacy switch RPCRequest(
...
Done Criteria:
Core sends target_boundary_time_ms in every legacy switch RPC request based on schedule.

---

Completion (2026-02-01): air_switch_to_live(..., target_boundary_time_ms). Legacy producer path (retired) previously used switch_to_live(segment_end); see Phase8DecommissionContract.
