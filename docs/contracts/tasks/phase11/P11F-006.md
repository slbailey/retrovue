...
1. Store plan-derived boundary when planning:
   ...
   def _plan_boundary(self, boundary_time: datetime) -> None:
       ...
       # Schedule LoadPreview and switch issuance
...
| **Implementation** | _plan_boundary_ms set when _segment_end_time_utc is set (start() and _handle_switch_complete); cleared in _transition_boundary_state when new_state == NONE. In _on_switch_issue_deadline, target_boundary_ms validated against _plan_boundary_ms; mismatch → FAILED_TERMINAL and _pending_fatal. |
