...
Instructions:
- Add `issued_at_time_ms` to SwitchToLiveRequest proto
...
Core populates `issued_at_time_ms` at the moment SwitchToLive is issued. AIR uses this value for lead-time feasibility. If `issued_at_time_ms` is absent or zero, receipt-time evaluation applies for backward compatibility only; this mode is deprecated and MUST NOT be relied upon by Core.
...
Core issues SwitchToLive at exactly `boundary_time - MIN_PREFEED_LEAD_TIME`. AIR evaluates lead time using receipt time. Any RPC latency causes:
...
5. Clock skew detection logging:
   ...
   if (issued_at_time_ms > 0) {
     ...
     if (transport_delta_ms < -1000 || transport_delta_ms > 1000) {
       std::cout << "[SwitchToLive] WARN: Clock skew detected" << std::endl;
     }
   }
   ```
