...
This test validates the protocol consequence of the hierarchy: when Core declares a time it can't meet (late prefeed), AIR returns PROTOCOL_VIOLATION — not NOT_READY. NOT_READY implies "retry until ready" which inverts the hierarchy.

Rule Definition (INV-CONTROL-NO-POLL-001):
legacy switch RPC MUST NOT return NOT_READY requiring Core to poll/retry. If prefeed timing was violated, return PROTOCOL_VIOLATION.

Test Scenario:
TEST_INV_CONTROL_NO_POLL_001_LatePrefeedReturnsProtocolViolation

- Given: legacy switch RPC issued with target boundary less than MIN_PREFEED_LEAD_TIME in future
...
TEST_F(PrefeedProtocolTests, LatePrefeedReturnsProtocolViolation) {
    ...
    // Act: Issue legacy switch RPC with insufficient lead time
    legacy switch RPCRequest request;
    request.set_channel_id(channel_id_);
    request.set_target_boundary_time_ms(target_boundary_ms);

    legacy switch RPCResponse response;
    auto status = playout_engine_->legacy switch RPC(request, &response);
...
TEST_F(PrefeedProtocolTests, SufficientLeadTimeAccepted) {
    ...
    playout_engine_->legacy preload RPC(channel_id_, "test_plan");
...
class TestPrefeedProtocol:
    ...
    async def test_protocol_violation_raises_exception(self, mock_air_client):
        ...
        mock_air_client.switch_to_live.return_value = legacy switch RPCResponse(
...
---
**Done:** pkg/air/tests/contracts/PrefeedProtocolTests.cpp: LatePrefeedReturnsProtocolViolation (full engine, asset required; skips if /opt/retrovue/assets/SampleA.mp4 missing), SufficientLeadTimeControlSurfaceSucceeds (control_surface path). Core no-retry verified by P11D-005 implementation.
