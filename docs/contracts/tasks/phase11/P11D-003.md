...
---
**Done:** Implemented in ExecuteSwitchAtDeadline(): when !ready, calls program_output->SetNoContentSegment(true) which triggers pad frame emission (existing LAW-OUTPUT-LIVENESS mechanism). Logs "Safety rails engaged for channel X". Metric: IncrementSwitchDeadlineNotReady(channel_id).
