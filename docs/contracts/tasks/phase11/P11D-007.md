...
protected:
    void SetUp() override {
        ...
        playout_engine_->legacy preload RPC(channel_id_, test_plan_handle_);
        WaitForPreviewReady();
    }
...
TEST_F(DeadlineSwitchTests, SwitchWithinOneFrame) {
    ...
    // Act: Issue legacy switch RPC with target boundary
...
TEST_F(DeadlineSwitchTests, MultiSwitchBoundaryAccuracy) {
    ...
    for (int64_t boundary : boundaries) {
        // Load fresh preview
        playout_engine_->legacy preload RPC(channel_id_, "plan_" + std::to_string(boundary));
        WaitForPreviewReady();
        ...
        legacy switch RPCResponse response;
        playout_engine_->legacy switch RPC(request, &response);
...
---
**Done:** pkg/air/tests/contracts/DeadlineSwitchTests.cpp: SwitchWithinOneFrame (full engine, asset required; skips if /opt/retrovue/assets/SampleA.mp4 missing; asserts |actual_switch_completion_time_ms - target| <= 33ms). SwitchAtDeadlineControlSurfaceAcceptsTarget (control_surface path). ExecuteSwitchAtDeadline now returns actual completion time (NowUtc/1000) for tolerance assertion.
