...
Instructions:
- Deprecate NOT_READY response from legacy switch RPC
...
1. Update legacy switch RPCResponse proto:
   ```protobuf
   enum SwitchStatus {
   ...
   message legacy switch RPCResponse {
...
2. Update legacy switch RPC handler:
   ```cpp
   grpc::Status legacy switch RPC(ServerContext* context,
                              const legacy switch RPCRequest* request,
       ...
       if (target_time_ms > 0) {
           ...
           if (lead_time_ms < MIN_PREFEED_LEAD_TIME_MS) {
               ...
               LOG_ERROR("[PlayoutService] INV-CONTROL-NO-POLL-001 VIOLATION: "
                         "legacy switch RPC received with insufficient lead time (%ld ms < %ld ms required)",
...
Protocol violation:
```
[PlayoutService] INV-CONTROL-NO-POLL-001 VIOLATION: legacy switch RPC received with insufficient lead time (500 ms < 5000 ms required)
```
...
Done Criteria:
legacy switch RPC in deadline-authoritative mode never returns NOT_READY; late prefeed returns PROTOCOL_VIOLATION.

---
**Done:** Proto: added violation_reason (field 7) to legacy switch RPCResponse. PlayoutEngine: kMinPrefeedLeadTimeMs=5000, lead-time check at start of deadline path returns kProtocolViolation with violation_reason. PlayoutInterface/playout_service forward violation_reason.
