...
Instructions:
- Create integration test verifying target_boundary_time_ms flows Core→AIR
...
- When: Core issues legacy switch RPC
- Then: legacy switch RPCRequest contains target_boundary_time_ms = T
- And: AIR logs receipt of target_boundary_time_ms = T
- And: legacy switch RPCResponse contains switch_completion_time_ms
...
TEST(BoundaryDeclarationTests, TargetFlowsFromCoreToAir) {
    ...
    // Act: Issue legacy switch RPC via gRPC
    ...
    auto status = stub_->legacy switch RPC(&context, request, &response);
    ...
    EXPECT_TRUE(LogContains("INV-BOUNDARY-DECLARED-001: legacy switch RPC received with target_boundary_time_ms=" +
...
Completion (2026-02-01): tests/contracts/BoundaryDeclarationTests.cpp added; TargetFlowsFromCoreToAir calls legacy switch RPC with target_boundary_time_ms and asserts switch_completion_time_ms > 0.
