...
Instructions:
- Create contract test verifying all legacy preload RPC calls have ≥MIN_PREFEED_LEAD_TIME_MS
...
1. TEST_INV_CONTROL_NO_POLL_001_NormalScheduling
   ...
   - Then: All legacy preload RPC calls have ≥MIN_PREFEED_LEAD_TIME
...
2. TEST_INV_CONTROL_NO_POLL_001_LateStart
   ...
   - Then: Violation is logged but legacy preload RPC still issued

3. TEST_INV_CONTROL_NO_POLL_001_RapidBoundaries
   - Given: Multiple boundaries in quick succession
   - When: Scheduling tick runs
   - Then: All legacy preload RPC calls have correct lead time
...
class TestPrefeedTiming:
    ...
    async def test_normal_scheduling_has_sufficient_lead_time(
        ...
        # Assert: legacy preload RPC called with sufficient lead time
...
class TestPrefeedTimingEdgeCases:
    ...
    async def test_boundary_in_past_is_skipped(
        ...
        # Should not attempt legacy preload RPC for past boundary
        assert not mock_air_client.load_preview.called

    async def test_duplicate_load_preview_prevented(
        self, channel_manager, mock_schedule_manager, mock_air_client
    ):
        """legacy preload RPC should only be issued once per boundary."""
        now = datetime.now(timezone.utc)
...
Done Criteria:
Contract tests prove all legacy preload RPC calls have ≥MIN_PREFEED_LEAD_TIME_MS except for documented edge cases.
