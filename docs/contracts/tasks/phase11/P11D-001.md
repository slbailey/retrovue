...
1. When legacy switch RPC received with target > 0:
   ```cpp
...
---
**Done:** Implemented: when target_boundary_time_ms > 0, lead-time check then lock release, MasterClock::WaitUntilUtcUs(target_us), re-lock, ExecuteSwitchAtDeadline(channel_id, target, lock). Returns immediately after wait with switch at deadline.
