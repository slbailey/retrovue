...
SwitchToLive issuance MUST be deadline-scheduled via `loop.call_later()`, not `threading.Timer`. This keeps all timer logic inside the event loop and avoids cross-thread issues.
