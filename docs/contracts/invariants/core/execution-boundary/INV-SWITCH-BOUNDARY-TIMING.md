...
## Required Tests
- `server/tests/contracts/test_inv_switch_boundary_timing.py` (Core declares boundary in protocol)
- `runtime/tests/contracts/DeadlineSwitchTests.cpp` (AIR executes switch within one frame of boundary)

## Enforcement Evidence

**Core (protocol boundary):**
- `server/tests/contracts/test_inv_switch_boundary_timing.py` — Verifies `SwitchToLiveRequest` includes `target_boundary_time_ms` and `issued_at_time_ms` fields, and that the boundary timestamp is >= issuance timestamp.

**AIR (execution boundary):**
- `runtime/tests/contracts/DeadlineSwitchTests.cpp` — Verifies switch completes within one frame duration (33 ms at 30 fps) of the declared boundary.
- `runtime/src/runtime/PlayoutEngine.cpp` (lines 834–968) — Runtime enforcement:
