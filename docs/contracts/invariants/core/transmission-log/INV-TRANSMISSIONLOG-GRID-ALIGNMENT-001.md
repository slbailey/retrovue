# INV-TRANSMISSIONLOG-GRID-ALIGNMENT-001 — PlaylistEvent boundaries must align to the channel grid
...
Ensures PlaylistEvents carry forward the grid alignment guarantee from ResolvedScheduleDay. Off-grid PlaylistEvent boundaries produce off-grid ExecutionEntry fences at the next derivation step, cascading a `LAW-GRID` violation into the runtime layer silently. This invariant is the PlaylistEvent-layer checkpoint in the grid integrity cascade.

## Guarantee

All PlaylistEvent `start_time` and `end_time` values must align to the channel's valid grid boundaries (`grid_block_minutes`, `block_start_offsets_minutes`, `programming_day_start`).
