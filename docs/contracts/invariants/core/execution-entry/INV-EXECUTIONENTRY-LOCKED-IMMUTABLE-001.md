...
- `server/tests/contracts/test_scheduling_constitution.py::TestInvExecutionentryLockedImmutable001` (PLAYLOG-IMMUT-001, PLAYLOG-IMMUT-002, PLAYLOG-IMMUT-003)

## Enforcement Evidence

`ExecutionWindowStore.replace_entry()` in `server/src/retrovue/runtime/execution_window_store.py` — enforces two immutability guards: (1) past-window entries (`end_utc_ms <= now_utc_ms`) are unconditionally rejected, (2) locked entries require an `override_record_id` for mutation. Raises `ValueError` with tag `INV-EXECUTIONENTRY-LOCKED-IMMUTABLE-001-VIOLATED` and window status (`"past"` or `"locked"`) on violation.
