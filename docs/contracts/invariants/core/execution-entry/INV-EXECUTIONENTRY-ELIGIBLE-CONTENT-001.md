...
At each rolling-window extension, HorizonManager MUST verify eligibility of all assets being added to the active window. If an asset has become ineligible since its PlaylistEvent was generated, the entry MUST be replaced with a declared filler and the violation MUST be logged (asset ID, channel ID, ineligibility reason). Silent use of ineligible content is unconditionally prohibited.
...
**Runtime fault.** The asset's eligibility changed after the PlaylistEvent was derived. HorizonManager must detect and handle this; it is not a planning error.
...
- **Rolling-window verification:** `HorizonManager` is responsible for re-verifying eligibility at each rolling-window extension — an asset that became ineligible after its `PlaylistEvent` was generated must be replaced with declared filler.
