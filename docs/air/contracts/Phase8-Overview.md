...
- **In scope (8.0–8.4):** Stream FD handoff, opaque bytes over HTTP (8.0); Air-owned ffmpeg TS output (8.1); start_offset_ms / hard_stop_time_ms driving ffmpeg (8.2); seamless SwitchToLive in the TS path (8.3); multiple readers and last-viewer teardown (8.4).
- **Explicitly out of scope until after 8.4:** Performance tuning, multi-channel scale, metrics on the stream path.
...
- **One logical stream per channel:** One write side (Air) and one or more read sides (HTTP viewers). Fan-out is in Python (8.4).
- **Clean shutdown:** When the last viewer disconnects, Air stops writing; no leaked FDs, no zombie ffmpeg (8.4).
...
| 8.4     | Fan-out & teardown             | Yes              | N viewers; last disconnect → Air stops; no leak |
