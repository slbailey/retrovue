...
**Principle:** Prove that Python can open a stream, Air can write bytes, and Python can serve them over HTTP—without ffmpeg, TS, or VLC. This is plumbing only.
...
- **Any bytes** (e.g. `"HELLO\n"` repeated). No TS, no codec.

## How it maps to the Phase 8 ladder

### Phase 8.0 (no media)

- **Python:** creates UDS server at e.g. `/tmp/retrovue/ch_mock.sock`; starts HTTP endpoint `/channels/mock.ts`.
- **Air:** StartChannel(1, …); AttachStream(channel_id=1, transport=UDS, endpoint=path); on successful connect → write `"HELLO\n"` then keep streaming bytes (or close for test).
- **Test:** GET /channels/mock.ts reads bytes and asserts body contains `HELLO\n`.
...
5. **Assert:** HTTP 200; bytes arrive; byte content matches what Air wrote (e.g. `"HELLO\n"`).
