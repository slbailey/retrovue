...
| **Web/API surfaces (experimental / legacy)** | HTTP entrypoints used for dev demos and runtime surfaces | FastAPI apps (varies) | `docs/core/architecture/ArchitectureOverview.md` (context) | `pkg/core/src/retrovue/runtime/program_director.py` (embedded FastAPI server). **Note:** ChannelManagerDaemon’s HTTP has been collapsed into ProgramDirector; there is no separate daemon (see [PD–CM collapse](core/ProgramDirector-ChannelManagerDaemon-Collapse.md)). IPTV playlist generation lives in `pkg/core/src/retrovue/web/iptv.py`. |

### Internal playout engine (C++) — real-time playout engine

| Component | Owns | Primary interfaces | Where to start (docs) | Where to start (code) |
| --- | --- | --- | --- | --- |
| **PlayoutEngine (control plane + engine)** | Channel lifecycle: start/stop/update plan; coordinates internal pipeline. Does not persist, segment, or store output; emits live bytes only. | gRPC service surface; Prometheus metrics | `pkg/air/docs/contracts/architecture/PlayoutEngineContract.md` • `pkg/air/docs/archive/domain/PlayoutEngineDomain.md` | `pkg/air/src/runtime/PlayoutEngine.cpp` • `pkg/air/src/runtime/PlayoutInterface.cpp` |
...
| **Renderer** | Converts staged frames into renderable output; optional preview | Internal C++ interface; telemetry | `pkg/air/docs/contracts/architecture/RendererContract.md` | `pkg/air/src/renderer/ProgramOutput.cpp` |
