...
TEST(LookaheadContract, TickThread_NeverCallsVideoDecodeAPIs) {
  ...
  // Start fill thread.
  buf.StartFilling(&prod, nullptr, DeriveRationalFPS(30.0), DeriveRationalFPS(30.0), &stop);
...
TEST(LookaheadContract, TickThread_NeverCallsAudioDecodeAPIs) {
  ...
  vbuf.StartFilling(&prod, &abuf, DeriveRationalFPS(30.0), DeriveRationalFPS(30.0), &stop);
...
TEST(LookaheadContract, FenceTick_DeliversNextBlock_ExactIndex) {
  ...
  buf.StartFilling(&block_a, &abuf, DeriveRationalFPS(30.0), DeriveRationalFPS(30.0), &stop);
  ASSERT_TRUE(WaitFor([&] { return buf.DepthFrames() >= 10; },
                       std::chrono::milliseconds(1000)));
  ...
  buf.StartFilling(&block_b, &abuf, DeriveRationalFPS(30.0), DeriveRationalFPS(30.0), &stop);
...
TEST(LookaheadContract, FenceTick_AudioAvailableFromNewBlock) {
  ...
  // Fill with block A.
  vbuf.StartFilling(&block_a, &abuf, DeriveRationalFPS(30.0), DeriveRationalFPS(30.0), &stop);
  ASSERT_TRUE(WaitFor([&] { return vbuf.DepthFrames() >= 10; },
                       std::chrono::milliseconds(1000)));
  ...
  vbuf.StartFilling(&block_b, &abuf, DeriveRationalFPS(30.0), DeriveRationalFPS(30.0), &stop);
...
TEST(LookaheadContract, FenceTick_RapidTransitions_Stable) {
  ...
  for (int block_idx = 0; block_idx < 5; block_idx++) {
    ...
    buf.StartFilling(&prod, &abuf, DeriveRationalFPS(30.0), DeriveRationalFPS(30.0), &stop);
