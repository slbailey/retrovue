...
_Related: [ProducerBus (Input Bus) Contract](ProducerBusContract.md) · [FileProducer Contract](../semantics/FileProducerContract.md) · [Output Continuity](../semantics/OutputContinuityContract.md) · [Playout Engine Contract](../semantics/PlayoutEngineContract.md)_
...
**Authoritative definition of the output liveness law** (ProgramOutput never blocks; if no content → deterministic pad black + silence) **lives in [PlayoutInvariants-BroadcastGradeGuarantees.md](../laws/PlayoutInvariants-BroadcastGradeGuarantees.md).**
...
- **Video:** Valid black frames at the PlayoutInstance’s ProgramFormat (width, height, frame rate). Frames are decoder-legal and suitable for encoding/mux; PTS/DTS advance monotonically (see [OutputContinuityContract](../semantics/OutputContinuityContract.md)).
...
- [FileProducerContract](../semantics/FileProducerContract.md) — Content producer contract (segment params, lifecycle).
- [OutputContinuityContract](../semantics/OutputContinuityContract.md) — PTS/DTS monotonicity for all output, including black.
