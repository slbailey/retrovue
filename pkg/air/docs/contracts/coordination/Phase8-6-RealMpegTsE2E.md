...
_Related: [Phase Model](../PHASE_MODEL.md) · [Phase 8 Overview](Phase8-Overview.md) · [Phase8-0 Transport](Phase8-0-Transport.md) · [Phase8-1 Air Owns MPEG-TS](Phase8-1-AirOwnsMpegTs.md) · [Phase8-4 Persistent MPEG-TS Mux](Phase8-4-PersistentMpegTsMux.md) · [Phase8-5 Fan-out & Teardown](Phase8-5-FanoutTeardown.md)_
...
- **AttachStream:** Connects to the stream endpoint (e.g. UDS) and retains the FD. It MUST NOT start a thread that writes dummy bytes (e.g. HELLO) to the FD. The stream may be silent until **legacy switch RPC** (or equivalent) starts the real TS mux.
- **After legacy switch RPC:** The only writer to the stream FD is the persistent MPEG-TS mux (EncoderPipeline, or equivalent). All bytes written are valid MPEG-TS (sync 0x47, 188-byte packets, PAT/PMT and PES as in 8.4).
...
- **Blocking before live:** Until legacy switch RPC, reads on the stream FD may block or return zero bytes; this is expected and MUST NOT be treated as an error. (Browsers, curl, and VLC behave differently; implementers must not add placeholder bytes to “fix” this.)
...
- **Sequence:** StartChannel → legacy preload RPC(asset_path, …) → AttachStream(transport, endpoint) → legacy switch RPC.
...
- legacy switch RPC: as today, start the TS mux writer thread (FfmpegLoop / EncoderPipeline) that reads frames from the live producer and writes MPEG-TS to the stored FD.
- Until legacy switch RPC, the stream may have no data; clients (e.g. browser or VLC) may block on read until the first TS bytes. This is acceptable.

## Tests

- **Unit / contract:** With real mux enabled, assert that no HELLO (or other non-TS) bytes are written to the stream FD after legacy switch RPC; optionally sample first N bytes and assert MPEG-TS structure (0x47, 188-byte packets).
- **E2E:** Start channel, legacy preload RPC, AttachStream, legacy switch RPC; GET the stream URL; assert HTTP 200, Content-Type video/mp2t, and that the body starts with valid TS (sync byte, packet size). Manual: open same URL in VLC and confirm playback.

## Exit criteria

- **No fake mux on the normal path:** Stream FD receives only real MPEG-TS from Air after legacy switch RPC.
...
2. Run the sequence: StartChannel → legacy preload RPC(asset_path, …) → AttachStream(UDS) → legacy switch RPC.
