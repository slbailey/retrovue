...
- Core issues intents via gRPC: `StartChannel`, `legacy preload RPC`, `legacy switch RPC`, `UpdatePlan`, `StopChannel`, `AttachStream`, `DetachStream`.
...
Air MUST log receipt time, effective time (if scheduled), and completion time for legacy preload RPC and legacy switch RPC.

| RPC | Receipt time | Effective time | Completion time |
|-----|--------------|----------------|-----------------|
| legacy preload RPC | When gRPC handler is invoked | When segment is installed into preview (or N/A if immediate) | When handler returns |
| legacy switch RPC | When gRPC handler is invoked | When switch is committed (preview promoted to live) | When handler returns |
...
- When a producer reaches its hard-stop boundary and Air clamps output (stops emitting frames from that producer), Air MUST log:
  ...
  3. **Clamp ended:** When Core issues the next intent (e.g. legacy switch RPC) and clamp is released.

- Logs MUST include: `channel_id`, `asset_path` (or segment id), `boundary_ms`, `correlation_id` (of the legacy preload RPC that established the segment).
...
|-------|-----------------|--------------|
| `AIR-INTENT-RECEIVED` | `correlation_id`, `rpc`, `channel_id`, `receipt_time_ms`, `asset_path` (legacy preload RPC), `start_frame`, `frame_count` (legacy preload RPC) | On entry to each gRPC handler |
...
| `AIR-CLAMP-ENDED` | `channel_id`, `asset_path`, `boundary_ms`, `next_correlation_id` | When legacy switch RPC (or next legacy preload RPC) ends clamp |
...
```
# Core (ChannelManager)
[CORE] CORE-INTENT-ISSUED correlation_id=ch1-sw42-1738340123456 rpc=legacy preload RPC channel_id=1 asset_path=/media/ep2.mp4 start_frame=0 frame_count=1800 issued_time_ms=1738340123456

[CORE] CORE-INTENT-ISSUED correlation_id=ch1-sw43-1738340153456 rpc=legacy switch RPC channel_id=1 issued_time_ms=1738340153456

# Air (PlayoutEngine)
[AIR] AIR-INTENT-RECEIVED correlation_id=ch1-sw42-1738340123456 rpc=legacy preload RPC channel_id=1 asset_path=/media/ep2.mp4 start_frame=0 frame_count=1800 receipt_time_ms=1738340123458
...
[AIR] AIR-INTENT-RESPONSE correlation_id=ch1-sw42-1738340123456 rpc=legacy preload RPC channel_id=1 success=true result_code=RESULT_CODE_OK completion_time_ms=1738340123482

[AIR] AIR-INTENT-RECEIVED correlation_id=ch1-sw43-1738340153456 rpc=legacy switch RPC channel_id=1 receipt_time_ms=1738340153458

[AIR] AIR-SWITCHTOLIVE-EFFECTIVE correlation_id=ch1-sw43-1738340153456 channel_id=1 effective_time_ms=1738340153462

[AIR] AIR-INTENT-RESPONSE correlation_id=ch1-sw43-1738340153456 rpc=legacy switch RPC channel_id=1 success=true result_code=RESULT_CODE_OK pts_contiguous=true completion_time_ms=1738340153465
```
...
```
# Core
[CORE] CORE-INTENT-ISSUED correlation_id=ch1-sw44-1738340183456 rpc=legacy preload RPC channel_id=1 asset_path=/media/ep3.mp4 start_frame=0 frame_count=1800 issued_time_ms=1738340183456

# Air (preview loaded; live segment hits hard stop before legacy switch RPC)
[AIR] AIR-INTENT-RECEIVED correlation_id=ch1-sw44-1738340183456 rpc=legacy preload RPC channel_id=1 asset_path=/media/ep3.mp4 receipt_time_ms=1738340183458
...
[AIR] AIR-INTENT-RESPONSE correlation_id=ch1-sw44-1738340183456 rpc=legacy preload RPC channel_id=1 success=true result_code=RESULT_CODE_OK completion_time_ms=1738340183482
...
# ... Core issues legacy switch RPC a few seconds later ...

[CORE] CORE-INTENT-ISSUED correlation_id=ch1-sw45-1738340186500 rpc=legacy switch RPC channel_id=1 issued_time_ms=1738340186500

[AIR] AIR-INTENT-RECEIVED correlation_id=ch1-sw45-1738340186500 rpc=legacy switch RPC channel_id=1 receipt_time_ms=1738340186502
...
[AIR] AIR-INTENT-RESPONSE correlation_id=ch1-sw45-1738340186500 rpc=legacy switch RPC channel_id=1 success=true result_code=RESULT_CODE_OK completion_time_ms=1738340186512
```
...
| LAW-OBS-004 | Timing evidence: receipt, effective, completion for legacy preload RPC and legacy switch RPC |
