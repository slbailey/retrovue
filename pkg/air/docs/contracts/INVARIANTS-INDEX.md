...
| **INV-SEAM-SEGMENT-PREFILL-001** | Early B-side pipeline creation — when SeamPreparer completes a segment result, B-side buffers and fill loop MUST be created immediately (not deferred to seam tick), so the swap gate evaluates a warm pipeline with full runway depth. Derives from INV-SEAM-006, INV-SEAM-SEG-003. Contract: [../../../../docs/contracts/invariants/air/INV-SEAM-SEGMENT-PREFILL-001.md](../../../../docs/contracts/invariants/air/INV-SEAM-SEGMENT-PREFILL-001.md). Test: SegmentPrefillContractTests. | `PipelineManager` | Coordination (Broadcast-Grade) |
| **INV-SEAM-SWAP-READINESS-001** | Swap readiness gate — incoming buffer MUST reach its configured target depth before swap, so the transition occurs into a warm pipeline at its designed operating point. PAD segments are exempt (video on-demand). Derives from INV-SEAM-002, INV-VIDEO-LOOKAHEAD-001. Contract: [../../../../docs/contracts/invariants/air/INV-SEAM-SWAP-READINESS-001.md](../../../../docs/contracts/invariants/air/INV-SEAM-SWAP-READINESS-001.md). Test: SegmentSwapReadinessContractTests. | `PipelineManager` | Coordination (Broadcast-Grade) |
...
| **INV-HANDOFF-001** | Emitted source frame must match selection: `actual_src_emitted == selected_src` (SourceFrameForTick) every output tick. Violation → Error log. Trace and root cause: [INV-HANDOFF-001-SOURCE-FRAME-TRACE.md](../../design/INV-HANDOFF-001-SOURCE-FRAME-TRACE.md). | `PipelineManager` | Semantic |
| **INV-HANDOFF-002** | Priming must not advance the producer output frame index. After `PrimeFirstTick()`, `frame_index_` must be 0; first frame pushed to LIVE_VIDEO_BUFFER must align with first content tick `selected_src`. Contract: [INV-HANDOFF-002-PRIMING-DOES-NOT-ADVANCE-FRAME-INDEX.md](INV-HANDOFF-002-PRIMING-DOES-NOT-ADVANCE-FRAME-INDEX.md). Tests: test_prime_first_tick_does_not_advance_frame_index, test_first_live_frame_matches_scheduler. | `TickProducer` | Semantic |
| **INV-FRAME-ALIGNMENT-001** | Frame selection alignment: `actual_src_emitted <= selected_src` every tick; if real frame emitted then `actual_src_emitted == selected_src`. Decoder may run ahead; consumer must never emit future frame. Violation → FRAME_ALIGNMENT_VIOLATION Error log. Contract: [playout/frame_selection_alignment.md](playout/frame_selection_alignment.md). Test: test_frame_alignment_invariant. | `PipelineManager` | Semantic |
