<!-- ⚠️ Historical document. Superseded by: [MetricsAndTimingContract](../../contracts/architecture/MetricsAndTimingContract.md) -->
...
2. **Frame Synchronization**: FFmpegDecoder, FrameRingBuffer, and ProgramOutput components align their timing to MasterClock timestamps, not system wall clock
...
2. **Component Time Alignment**: FFmpegDecoder, FrameRingBuffer, and ProgramOutput components must all align their timing to MasterClock timestamps, not system wall clock.
...
```cpp
bool ProgramOutput::pull_frame(int channel_id, Frame& out_frame) {
...
- [RetroVue MasterClock](../../../retrovue_core/docs/domain/MasterClock.md) — Runtime clock specification
