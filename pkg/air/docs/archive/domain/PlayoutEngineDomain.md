<!-- ⚠️ Historical document. Superseded by: [PlayoutEngineContract](../../contracts/architecture/PlayoutEngineContract.md) and [Air Architecture Reference](../../contracts/AirArchitectureReference.md) -->
...
- One `ProgramOutput` reference (output consumer, receives frames from live slot only)
...
3. **Consume**: Frame is popped by `ProgramOutput` for output
...
### 4. Program Output (Renderer)

**Purpose**: Consumer interface that pulls decoded frames from the playout engine and packages them for output (MPEG-TS, preview window, etc.). Broadcast-native term: **ProgramOutput**.
...
public:
    ...
    // Check if output is ready to accept frames
...
- OutputBus routing: Frames flow to attached OutputSink (e.g. MpegTSOutputSink)
- `PreviewProgramOutput`: Displays frames in debug window (OpenGL/SDL)
- `HeadlessProgramOutput`: Consumes frames without display (production/testing)
...
3. **Channel → ProgramOutput** (N:1 reference)

   - Multiple channels may share one ProgramOutput
   - ProgramOutput is owned by playout service
   - Channel holds weak reference
   - ProgramOutput only consumes frames from live slot producer
