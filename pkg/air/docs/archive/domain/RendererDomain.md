<!-- ⚠️ Historical document. Superseded by: [RendererContract](../../contracts/architecture/RendererContract.md) -->
...
| Aspect           | HeadlessProgramOutput  | PreviewProgramOutput               |
| ---------------- | ---------------------- | ---------------------------------- |
...
| **Fallback**     | N/A                    | HeadlessProgramOutput if SDL2 missing |
...
| **SDL2 Unavailable** | Initialize fails        | Fallback to HeadlessProgramOutput      | No visual output     |
...
```
FrameRouter                        FrameRingBuffer                    ProgramOutput
┌─────────────────┐               ┌─────────────────┐               ┌──────────────────┐
...
StartChannel(plan_handle):
    ...
    5. Create & start ProgramOutput (output thread)   ← ProgramOutput enters here
    6. Update metrics
...
UpdatePlan(new_plan_handle):
    1. Stop ProgramOutput                            ← Stop consumer first
    2. Stop FrameProducer
    3. Clear buffer
    4. Restart FrameProducer with new plan
    5. Restart ProgramOutput                         ← Restart consumer
...
| ---------------- | ------------------------------------------------ |
| **Creation**     | `std::thread` in `ProgramOutput::Start()`         |
| **Entry Point**  | `ProgramOutput::RenderLoop()` (protected method) |
...
std::atomic<bool> ProgramOutput::running_;
std::atomic<bool> ProgramOutput::stop_requested_;

// Statistics counters
std::atomic<uint64_t> ProgramOutput::frames_rendered_;
std::atomic<uint64_t> ProgramOutput::frames_skipped_;
```
...
```cpp
class GPURenderer : public ProgramOutput {
 protected:
...
```cpp
class SDIRenderer : public ProgramOutput {
 protected:
    void RenderFrame(const Frame& frame) override {
