...
## Reproduction
```bash
timeout 30 pkg/air/build/blockplan_contract_tests --gtest_filter="SeamContinuityEngineContractTest.*"
```
Exits with SEGFAULT (actually a timeout-killed hang, not a true segfault).

Individual subtests pass in isolation:
```bash
timeout 20 pkg/air/build/blockplan_contract_tests --gtest_filter="SeamContinuityEngineContractTest.T_SEAM_001a*"
# PASSED (3597 ms)
```
...
```bash
git stash -- pkg/air/src/blockplan/TickProducer.cpp \
             pkg/air/include/retrovue/blockplan/TickProducer.hpp \
             pkg/air/src/blockplan/PipelineManager.cpp
cmake --build pkg/air/build -j$(nproc)
timeout 15 pkg/air/build/blockplan_contract_tests --gtest_filter="SeamContinuityEngineContractTest.*"
