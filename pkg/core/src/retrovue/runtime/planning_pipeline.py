...
def fill_breaks(
    ...
    for block in segmented_blocks:
        ...
        # Unfilled break time is now handled as inline pad segments
        # within _interleave_segments, so block-level pad stays unchanged.
...
def _interleave_segments(
    ...
    for i, cs in enumerate(fb.content_segments):
        ...
        # with any leftover pad distributed evenly between ads
        # (INV-BREAK-PAD-DISTRIBUTED-001)
        if next_break is not None and i < len(fb.content_segments) - 1:
            num_items = len(next_break.items)
            break_gap_ms = next_break.allocated_ms - next_break.filled_ms

            if num_items > 0 and break_gap_ms > 0:
                # Distribute gap evenly; remainder goes to last gaps
                base_pad_ms = break_gap_ms // num_items
                extra_ms = break_gap_ms % num_items
                # Pads go BETWEEN items (after each item except possibly
                # adjusted so total is exact). We place a pad after every
                # item: N items → N pads, distributed evenly.
                pad_sizes = [base_pad_ms] * num_items
                # Distribute remainder one ms at a time from the end
                for r in range(extra_ms):
                    pad_sizes[num_items - 1 - r] += 1
            else:
                pad_sizes = [0] * max(num_items, 1)

            for idx, item in enumerate(next_break.items):
                ...
                seg_index += 1
                # Insert distributed pad after this item
                if pad_sizes[idx] > 0:
                    flat.append({
                        "segment_index": seg_index,
                        "segment_duration_ms": pad_sizes[idx],
                        "segment_type": "pad",
                    })
                    seg_index += 1
