...
REPO_ROOT = Path(__file__).resolve().parents[4]  # pkg/core/tests/contracts -> repo root
...
EXCLUDED_FILES = {
    ...
    "pkg/core/core/proto/execution_evidence_v1_pb2_grpc.py",
    "pkg/core/core/proto/retrovue/execution_evidence_v1_pb2_grpc.py",
    "pkg/core/core/proto/retrovue/playout_pb2_grpc.py",
    "pkg/core/core/proto/playout_pb2_grpc.py",
    # Enricher/importer templates and base classes (parameter names, not literal values)
    "pkg/core/src/retrovue/adapters/enrichers/enricher_template.py",
    "pkg/core/src/retrovue/adapters/enrichers/base.py",
    "pkg/core/src/retrovue/adapters/importers/importer_template.py",
    "pkg/core/src/retrovue/adapters/importers/base.py",
    "pkg/core/src/retrovue/adapters/importers/plex_importer.py",
    # CLI commands that accept --token / --api-key as flags (no literals)
    "pkg/core/src/retrovue/cli/commands/enricher.py",
    "pkg/core/src/retrovue/cli/commands/source.py",
    "pkg/core/src/retrovue/cli/commands/collection.py",
    "pkg/core/src/retrovue/cli/commands/_ops/backfill_plex_artwork.py",
    # Legacy CLI (parameter references only)
    "pkg/core/src_legacy/retrovue/cli/commands/plex.py",
    "pkg/core/src_legacy/retrovue/cli/commands/source.py",
    "pkg/core/src_legacy/retrovue/content_manager/ingest_orchestrator.py",
    "pkg/core/src_legacy/retrovue/content_manager/source_service.py",
    "pkg/core/src_legacy/retrovue/api/main.py",
    "pkg/core/src_legacy/retrovue/api/web/pages.py",
    # Settings module (reads from env, declares field names)
    "pkg/core/src/retrovue/infra/settings.py",
    # Logging module (redaction patterns, not secrets)
    "pkg/core/src/retrovue/infra/logging.py",
    # Test fixtures with fake tokens
    "pkg/core/tests/contracts/plex/test_plex_artwork.py",
    "pkg/core/src/retrovue/adapters/registry.py",
    # This test file itself
    "pkg/core/tests/contracts/test_inv_no_hardcoded_secrets.py",
    # GitHub CI (test-only postgres password)
    "pkg/core/.github/workflows/test-workflow.yml",
    # Alembic config (commented-out example DSN)
    "pkg/core/alembic.ini",
    # Legacy test fixtures with test:test placeholder creds (skipped)
    "pkg/core/tests/_legacy/test_compliance_reporting.py",
    "pkg/core/tests/_legacy/test_identity_model.py",
    # Contract conftest (documentation example in docstring)
    "pkg/core/tests/contracts/conftest.py",
