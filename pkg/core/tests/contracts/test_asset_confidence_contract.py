...
class TestAssetConfidenceContractCLI:
    def test_cli_reports_thresholds_and_buckets(self):
        runner = CliRunner()

        # Mock the ingest service to return a deterministic payload including thresholds and buckets
        ...
        with patch("retrovue.cli.commands.collection.session") as mock_session, \
            ...
            result = runner.invoke(app, ["container", "ingest", "Movies", "--json"]) 
            assert result.exit_code == 0

            data = json.loads(result.stdout)
            assert "thresholds" in data
            assert data["thresholds"]["auto_ready"] == 0.8
            assert data["thresholds"]["review"] == 0.5
            s = data["stats"]
            for key in ("assets_auto_ready", "assets_needs_enrichment", "assets_needs_review"):
                assert key in s
