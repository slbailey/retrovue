...
# ---------------------------------------------------------------------------
# (c) ROOT PYTEST COMPATIBILITY — sys.path includes pkg/core
# ---------------------------------------------------------------------------
...
class TestRootPytestCompat:
    ...
    def test_pkg_core_on_sys_path(self):
        """pkg/core must be on sys.path so 'from tests.contracts...' resolves."""
        pkg_core = str(Path(__file__).resolve().parents[2])
        assert pkg_core in sys.path, (
            f"pkg/core ({pkg_core}) not in sys.path — "
