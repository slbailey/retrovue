...
# ===========================================================================
# PSM-T006: Negative Frame Count Rejection  (INV-PSM-02)
# ===========================================================================

class TestPSMT006NegativeFrameCount:
    """load_playlist rejects Playlists with negative frame_count."""

    def _make_bad_playlist(self, frame_count: int = -1) -> Playlist:
        """Construct a Playlist with a segment having the given frame_count."""
        w_start = datetime(2026, 2, 7, 11, 0, 0, tzinfo=timezone.utc)
        w_end = datetime(2026, 2, 7, 12, 0, 0, tzinfo=timezone.utc)
        seg = PlaylistSegment(
            segment_id="seg-bad-001",
            start_at=w_start,
            duration_seconds=100.0,
            type="PROGRAM",
            asset_id="asset-bad",
            asset_path="/mnt/media/bad.mp4",
            frame_count=frame_count,
        )
        return Playlist(
            channel_id=CHANNEL_ID,
    ...
    def _make_channel_manager(self):
        """Build a minimal ChannelManager for validation testing."""
        from unittest.mock import MagicMock

        from retrovue.runtime.channel_manager import ChannelManager
        from retrovue.runtime.clock import MasterClock as ConcreteMasterClock

        clock = ConcreteMasterClock()
        schedule_service = MagicMock()
        schedule_service.load_schedule = MagicMock(return_value=(True, None))
        schedule_service.get_playout_plan_now = MagicMock(return_value=[])
        program_director = MagicMock()
        program_director.global_mode = "normal"
    ...
    def test_negative_frame_count_raises_valueerror(self):
        cm = self._make_channel_manager()
        bad_pl = self._make_bad_playlist(frame_count=-1)

        with pytest.raises(ValueError, match="frame_count=-1"):
            cm.load_playlist(bad_pl)

    def test_playlist_unchanged_after_rejection(self):
        cm = self._make_channel_manager()

        # Load a valid playlist first.
        valid_seg = _make_segment(
            0,
            datetime(2026, 2, 7, 11, 0, 0, tzinfo=timezone.utc),
            PROGRAM_FRAMES,
        )
        valid_pl = Playlist(
            channel_id=CHANNEL_ID,
            channel_timezone=CHANNEL_TZ,
            window_start_at=datetime(2026, 2, 7, 11, 0, 0, tzinfo=timezone.utc),
            window_end_at=datetime(2026, 2, 7, 11, 22, 0, tzinfo=timezone.utc),
            generated_at=datetime(2026, 1, 1, tzinfo=timezone.utc),
            source="TEST",
            segments=(valid_seg,),
        )
        cm.load_playlist(valid_pl)
        assert cm._playlist is valid_pl

        # Attempt to load a bad playlist — should be rejected.
        bad_pl = self._make_bad_playlist(frame_count=-5)
        with pytest.raises(ValueError):
            cm.load_playlist(bad_pl)

        # Original playlist is still loaded.
        assert cm._playlist is valid_pl

    def test_zero_frame_count_accepted(self):
        """frame_count == 0 is valid (e.g. marker segment)."""
        cm = self._make_channel_manager()
        zero_pl = self._make_bad_playlist(frame_count=0)
        # Should not raise.
        cm.load_playlist(zero_pl)
        assert cm._playlist is zero_pl
