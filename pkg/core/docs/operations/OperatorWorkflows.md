...
3. Discover containers from that Source:
   - `retrovue source discover <source_id>`
4. View containers and enable ingest on specific ones:
   - `retrovue container list --source <source_id>`
   - `retrovue container update <container_id> --sync-enable --local-path /mnt/media/...`
5. Ingest:
   - `retrovue container ingest <container_id>`  
     or  
...
3. Attach it to the container with a priority:
   - `retrovue container attach-enricher <container_id> <enricher_id> --priority <n>`
...
### 5. Complete Container Reset (Nuclear Option)

When you need to completely start over with a container:

1. **Preview what will be deleted** (always do this first):
   - `retrovue container wipe "TV Shows" --dry-run`
2. **Actually wipe everything**:
   - `retrovue container wipe "TV Shows"`
3. **Re-discover the container** (if needed):
   - `retrovue source discover <source_id>`
4. **Re-ingest from scratch**:
   - `retrovue container ingest "TV Shows"`

**⚠️ WARNING**: This deletes ALL data for the container (assets, episodes, seasons, titles, review queue entries). The container and path mappings are preserved for re-ingest.

**Use cases**:

- Container has corrupted or inconsistent data
