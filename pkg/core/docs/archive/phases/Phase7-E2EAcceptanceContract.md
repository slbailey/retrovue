...
> **Historical:** This doc referred to ChannelManagerDaemon and `GET /channels/{channel_id}.ts`. Current architecture: ProgramDirector owns the CM registry; stream path is **GET /channel/{channel_id}.ts**. See `docs/core/ProgramDirector-ChannelManagerDaemon-Collapse.md`.
...
- **Startup**: Retrovue is started (e.g. `retrovue program-director start` with Phase 0 options); ProgramDirector runs (single process, owns ChannelManager registry); mock channel is available (e.g. `mock` or `retro1`).
- **Tune-in (Phase 7 only)**: A "viewer" is simulated by **HTTP** `GET /channel/{channel_id}.ts` (or equivalent) against ProgramDirector. Optionally read some bytes or only assert HTTP 200 and headers. (For Phases 0–6, use the direct ProgramDirector API instead.)
...
2. Use a **test script or pytest** to simulate tune-in (no browser, no human):
   - HTTP client: `GET http://localhost:8000/channel/mock.ts` (or chosen channel id).
...
- **E2E test (automated)**:
  - Start ProgramDirector in process or subprocess (e.g. pytest fixture with free port).
  - Simulate tune-in: `requests.get(http://localhost:{port}/channel/{channel_id}.ts, stream=True)` (or httpx); read first N bytes or only headers; then close.
