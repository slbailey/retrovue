...
- [x] **8c** — Extract raw TS fanout management into TsConsumptionAdapter._wire_fanout(channel_id, mgr) helper.
- [x] **8d** — Both adapters call PD.start_channel() as the single lifecycle entry point. Delete _ensure_channel_active_for_hls() body (~190 lines). Run tests (floor 330). Contract test from 8a must PASS.
- [x] **8e** — Update CLAUDE.md: "Channel lifecycle is PD-owned. HLS and TS are consumption adapters. There is one lifecycle path: start_channel(). Adapters add consumption-model behavior (phantom, fanout) but do not own lifecycle."
...
- [x] **8.5c** — Validate end-to-end: start a channel, join as viewer, leave, confirm full lifecycle traceable in logs. Commit with test.
- [x] **8.5d** — Update CLAUDE.md: "Runtime lifecycle transitions must emit structured log events at DEBUG level. Viewer sessions must carry a correlation ID traceable end-to-end."
...
- [x] **10h** — Rewrite test_inv_hls_discontinuity_marker.py
...
| 10h | 2026-03-29 | 8e0806f | Rewrite test_inv_hls_discontinuity_marker.py (13 tests): DISCONTINUITY-MARKER-001, RESTART-DISCONTINUITY-001; 357 pass, 2 pre-existing failures, 0 regressions |
