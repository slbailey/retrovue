...
- **With or without full process.** Option A: run ProgramDirector + ChannelManagerDaemon; simulate tune-in via **direct ProgramDirector API** (e.g. `ensure_channel_running(channel_id)`)—no HTTP. Advance time or wait until near boundary and assert that CM has issued LoadPreview (by prefeed deadline) and then SwitchToLive at the boundary. Option B: unit-test CM in isolation with a **stepped clock**: advance clock to just before boundary, **trigger CM evaluation** (tick/update), assert LoadPreview was sent before boundary and SwitchToLive at boundary.
